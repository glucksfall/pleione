...
def make_xml():
	...
	for ind in range(opts['pop_size']):
		...
		if opts['slurm'] is not None:
			cmd = os.path.expanduser('sbatch --no-requeue -p {partition} -N {nodes} -c {ncpus} -n {ntasks} -o {null} -e {null} -J {job_name} --wrap ""{make_xml}""'.format(**job_desc))
			cmd = re.findall(r'(?:[^\s,"]|"+(?:=|\\.|[^"])*"+)+', cmd)
			out, err = subprocess.Popen(cmd, shell = False, stdout = subprocess.PIPE, stderr = subprocess.PIPE).communicate()
		...
		# use multiprocessing.Pool
			...
			cmd = os.path.expanduser(job_desc['make_xml'])
			cmd = re.findall(r'(?:[^\s,"]|"+(?:=|\\.|[^"])*"+)+', cmd)
			squeue.append(cmd)
...
def simulate():
	...
	for ind in range(opts['pop_size']):
		for sim in range(opts['num_sims']):
			...
			job_desc['exec_nfsim'] = '{:s} -cb -csv -xml {:s}.xml -rnf {:s} -o {:s}'.format(opts['nfsim'], model, rnfile, output)
			...
			# use multiprocessing.Pool
				...
				cmd = os.path.expanduser(job_desc['exec_nfsim'])
				cmd = re.findall(r'(?:[^\s,"]|"+(?:=|\\.|[^"])*"+)+', cmd)
				squeue.append(cmd)
...
def evaluate():
	job_desc = {
		...
		'stdout'    : 'stdout_{:s}.txt'.format(opts['systime']),
		'stderr'    : 'stderr_{:s}.txt'.format(opts['systime']),
		'doerror'   : '{:s} -m pleione.nfsim-doerror --crit {:s}'.format(opts['python'], opts['crit_vals']),
		#'deverror'  : '{:s} -m pleione.nfsim-allerror --crit {:s}'.format(opts['python'], opts['crit_vals']),
		}
...
def backup():
	...
	cmd = 'find . -maxdepth 1 -type f ( -name model_*.rnf -o -name model_*.bngl -o -name model_*.xml ) -exec mv -t {:s} {{}} +'.format(folder_parsets)
	cmd = re.findall(r'(?:[^\s,"]|"+(?:=|\\.|[^"])*"+)+', cmd)
	out, err = subprocess.Popen(cmd, shell = False, stdout = subprocess.PIPE, stderr = subprocess.PIPE).communicate()
