...
def argsparser():
	...
	parser.add_argument('--steps'  , metavar = 'float', type = str  , required = True , nargs = 1  , help = 'time points to simulate')
...
def simulate():
	...
	for ind in range(opts['pop_size']):
		for sim in range(opts['num_sims']):
			...
			job_desc['exec_piskas'] = '{:s} -i {:s}.kappa -t {:s} -p {:s} -o {:s} -sync-t {:s}'.format( \
				opts['piskas'], model, opts['final'], opts['steps'], output, opts['sync'])

			# use SLURM Workload Manager
			if opts['slurm'] is not None:
				cmd = os.path.expanduser('sbatch --no-requeue -p {partition} -N {nodes} -c {ncpus} -n {ntasks} -o {null} -e {null} -J {job_name} --wrap ""mpirun {exec_piskas}""'.format(**job_desc))
				cmd = re.findall(r'(?:[^\s,"]|"+(?:=|\\.|[^"])*"+)+', cmd)
				out, err = subprocess.Popen(cmd, shell = False, stdout = subprocess.PIPE, stderr = subprocess.PIPE).communicate()
