...
__license__ = 'gpl-3.0'
__software__ = 'kasim-v3.5-250915'
...
def argsparser():
	...
	parser.add_argument('--kasim'  , metavar = 'path' , type = str  , required = False, default = '~/bin/kasim3'  , help = 'KaSim path, default ~/bin/kasim3')
...
def simulate():
	...
	for ind in range(0, opts['pop_size']):
		for sim in range(0, opts['num_sims']):
			...
			job_desc['exec_kasim'] = '{:s} -i {:s}.kappa -t {:s} -p {:s} -o {:s}'.format(opts['kasim'], model, opts['final'], opts['steps'], output)
