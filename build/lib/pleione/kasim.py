...
import argparse, glob, multiprocessing, os, random, re, subprocess, sys, time
import pandas, numpy

def safe_check():
	cmd = 'command -v {:s}'.format(opts['python'])
	cmd = re.findall(r'(?:[^\s,"]|"+(?:=|\\.|[^"])*"+)+', cmd)
	out, err = subprocess.Popen(cmd, shell = False, stdout = subprocess.PIPE, stderr = subprocess.PIPE).communicate()
	if err != b'':
		raise ValueError('python3 can\'t be called to perform calculation error')
