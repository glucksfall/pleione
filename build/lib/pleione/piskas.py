...
import argparse, glob, multiprocessing, os, re, sys, subprocess, time
import pandas, numpy
...
def argsparser():
	...
	# required arguments
	parser.add_argument('--model'  , metavar = 'str'  , type = str  , required = True , nargs = 1  , help = 'kappa model with tagged variables to parameterize')
	parser.add_argument('--final'  , metavar = 'float', type = str  , required = True , nargs = 1  , help = 'limit time to simulate')
	parser.add_argument('--steps'  , metavar = 'float', type = str  , required = True , nargs = 1  , help = 'time steps to simulate by KaSim')
...
def configurate():
	...
	# find compartments
	compartments = []
	regex = '%compartment: \'(\w+)\' ([-+]?(?:(?:\d*\.\d+)|(?:\d+\.?))(?:[Ee][+-]?\d+)?)\n'
	for line in range(len(data)):
		matched = re.match(regex, data[line])
		if matched:
			compartments.append(matched.group(1))
	...
	return parameters, compartments
...
def simulate():
	...
	for ind in range(opts['pop_size']):
		for sim in range(opts['num_sims']):
			model = population['model', ind]
			output = '{:s}.{:03d}.'.format(model, sim)

			job_desc['exec_piskas'] = '{:s} -i {:s}.kappa -t {:s} -p {:s} -o {:s} -sync-t {:s}'.format(opts['piskas'], model, opts['final'], opts['steps'], output, opts['sync'])

			# use SLURM Workload Manager
			if opts['slurm'] is not None:
				cmd = 'sbatch --no-requeue -p {partition} -N {nodes} -c {ncpus} -n {ntasks} -o {null} -e {null} -J {job_name} --wrap ""mpirun {exec_piskas}""'.format(**job_desc)
			...
			# use multiprocessing.Pool
				...
				cmd = os.path.expanduser('mpirun -np {:d} {:s}'.format(len(compartments), job_desc['exec_piskas']))
				cmd = re.findall(r'(?:[^\s,"]|"+(?:=|\\.|[^"])*"+)+', cmd)
				squeue.append(cmd)
...
def evaluate():
	job_desc = {
		...
		'stdout'    : 'stdout_{:s}.txt'.format(opts['systime']),
		'stderr'    : 'stderr_{:s}.txt'.format(opts['systime']),
		'doerror'   : '{:s} -m pleione.piskas-doerror --crit {:s}'.format(opts['python'], opts['crit_vals']),
		'deverror'  : '{:s} -m pleione.piskas-allerror --crit {:s}'.format(opts['python'], opts['crit_vals']),
		}
	...
	for ind in range(opts['pop_size']):
		for cpu in compartments:
			...
			sims = ' '.join(glob.glob('{:s}.*.{:s}.out'.format(model, cpu)))
...
def ranking():
	...
	with open('{:s}_{:03d}.txt'.format(opts['outfile'], iter), 'w') as file:
		...
		for name in range(len(fitfunc)):
			for cpu in compartments:
				file.write('{:s}\t'.format(fitfunc[name]))
			file.write('{:s}\t'.format(fitfunc[name]))
		for key in range(len(par_keys)):
			file.write('VALUE\t')
		file.write('\n')
...
def clean():
	cmd = 'find . -maxdepth 1 -type f ( -name log*txt -o -name *.bngl -o -name *.xml -o -name *.gdat -o -name *.species -o -name *.rnf -o -name model*.sh -o -name model*.kappa -o -name model*.out.txt -o -name model*.out -o -name model*.txt -o -name {:s}*.txt ) ! -name {:s} ! -name {:s} -exec rm {{}} +'.format(opts['outfile'], opts['model'], opts['crit_vals'])
	cmd = re.findall(r'(?:[^\s,"]|"+(?:=|\\.|[^"])*"+)+', cmd)
	out, err = subprocess.Popen(cmd, shell = False, stdout = subprocess.PIPE, stderr = subprocess.PIPE).communicate()
...
def backup():
	...
	cmd = 'find . -maxdepth 1 -type f -name model_*.out -exec mv -t {:s} {{}} +'.format(folder_rawdata)
	cmd = re.findall(r'(?:[^\s,"]|"+(?:=|\\.|[^"])*"+)+', cmd)
	out, err = subprocess.Popen(cmd, shell = False, stdout = subprocess.PIPE, stderr = subprocess.PIPE).communicate()

	# archive simulated models
	cmd = 'find . -maxdepth 1 -type f -name model_*.kappa -exec mv -t {:s} {{}} +'.format(folder_parsets)
	cmd = re.findall(r'(?:[^\s,"]|"+(?:=|\\.|[^"])*"+)+', cmd)
	out, err = subprocess.Popen(cmd, shell = False, stdout = subprocess.PIPE, stderr = subprocess.PIPE).communicate()
...
if __name__ == '__main__':
	...
	parameters, compartments = configurate()
